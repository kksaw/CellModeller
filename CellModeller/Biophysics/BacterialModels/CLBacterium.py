...
class CLBacterium:
    ...
    def __init__(self, simulator,
                 max_substeps=8,
                 max_cells=500000,   #kk: changed from 10000 to 50000
                 max_contacts=24,
                 max_planes=4,
                 max_pillars=200,    #kk: added
                 max_sqs=192**2,
                 grid_spacing=5.0,
                 muA = 1.0,
                 gamma=10.0,
        ...
        self.max_pillars = max_pillars      #kk: added
        ...
        self.n_cts = 0
        self.n_planes = 0
        self.n_pillars = 0      #kk: added
        
        self.ncells = numpy.zeros(1000,int)      #kk: added
        ...
        #NEED ANOTHER GRID???? OR SPECIFY GRIDMAX GRIDMIN SO DONT NEED TO UPDATE EVERY ROUND
    ...
    #kk: added
    def addPillar(self, pos=(0,0), dir=(0,0,1), rad=5.0, length=50.0, coeff=1.0):
        plidx = self.n_pillars
        self.n_pillars += 1
        self.pillar_centers[plidx] = tuple(pos+(0.500*length,0,))
        self.pillar_dirs[plidx] = tuple(dir+(0,))
        self.pillar_lens[plidx] = length
        self.pillar_rads[plidx] = rad
        self.pillar_coeffs[plidx] = coeff
        #self.pillar_sq[plidx] = sq #need???
        self.set_pillars()
        """uuuu"""
    ...
    def init_data(self):
        ...
        #kk: added: constraint pillars
        pillar_geom = (self.max_pillars,)
        self.pillar_centers = numpy.zeros(pillar_geom, vec.float4)
        self.pillar_centers_dev = cl_array.zeros(self.queue, pillar_geom, vec.float4)
        self.pillar_dirs = numpy.zeros(pillar_geom, vec.float4)
        self.pillar_dirs_dev = cl_array.zeros(self.queue, pillar_geom, vec.float4)
        self.pillar_lens = numpy.zeros(pillar_geom, numpy.float32)
        self.pillar_lens_dev = cl_array.zeros(self.queue, pillar_geom, numpy.float32)
        self.pillar_rads = numpy.zeros(pillar_geom, numpy.float32)
        self.pillar_rads_dev = cl_array.zeros(self.queue, pillar_geom, numpy.float32)
        self.pillar_coeffs = numpy.zeros(pillar_geom, numpy.float32)
        self.pillar_coeffs_dev = cl_array.zeros(self.queue, pillar_geom, numpy.float32)
    ...
    #kk: added
    def load_from_ncells(self, ncells):
        self.ncells = ncells
    ...
    def set_pillars(self):
        """Copy pillar centers, dirs, lens, rads and coeffs to the device from local."""
        self.pillar_centers_dev[0:self.n_pillars].set(self.pillar_centers[0:self.n_pillars])
        self.pillar_dirs_dev[0:self.n_pillars].set(self.pillar_dirs[0:self.n_pillars])
        self.pillar_lens_dev[0:self.n_pillars].set(self.pillar_lens[0:self.n_pillars])
        self.pillar_rads_dev[0:self.n_pillars].set(self.pillar_rads[0:self.n_pillars])
        self.pillar_coeffs_dev[0:self.n_pillars].set(self.pillar_coeffs[0:self.n_pillars])
    ...
    def progress_finalise(self):
        ...
        if self.simulator:
            ...
            for state in self.simulator.cellStates.values():
                self.updateCellState(state)
        self.store_ncells()     #kk: added
    
    #kk: added
    def store_ncells(self):
        self.ncells[self.frame_no] = self.n_cells
        return self.ncells
    ...
    def find_contacts(self, predict=True):
        ...
        #kk: added
        self.program.find_pillar_contacts(self.queue,
                                          (self.n_cells,),
                                          None,
                                          numpy.int32(self.max_cells),
                                          numpy.int32(self.max_contacts),
                                          numpy.int32(self.n_pillars),
                                          self.pillar_centers_dev.data,
                                          self.pillar_dirs_dev.data,
                                          self.pillar_lens_dev.data,
                                          self.pillar_rads_dev.data,
                                          self.pillar_coeffs_dev.data,
                                          centers.data,
                                          dirs.data,
                                          lens.data,
                                          self.cell_rads_dev.data,
