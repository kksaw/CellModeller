...
//sigh...
float pt_to_pillar_dist(float4 p, float4 c, float4 dir)
{
  return length((p-c) - (dot((p-c),dir)*dir));
}
...
// added
__kernel void find_pillar_contacts(const int max_cells,
                                   const int max_contacts,
                                   const int n_pillars,
                                   __global const float4* pillar_centers,
                                   __global const float4* pillar_dirs,
                                   __global const float* pillar_lens,
                                   __global const float* pillar_rads,
                                   __global const float* pillar_coeffs,
                                   __global const float4* cell_centers,
                                   ...
                                   __global float* stiff)
{
  ...
  // collision count
  int k = n_cts[i]; //existing contacts
  
  float4 end1 = cell_centers[i] - 0.5f*cell_lens[i]*cell_dirs[i]; // 'left' end of the cell
  float4 end2 = cell_centers[i] + 0.5f*cell_lens[i]*cell_dirs[i]; // 'right' end of the cell
  
  for (int n=0; n < n_pillars; n++) { //loop through all pillars
    int to1 = -2*n - 11;
    int to2 = -2*n - 11;

    //check for old contact with this pillar
    int cti1 = -1;
    int cti2 = -1;
    for (int m = i*max_contacts; m < i*max_contacts+n_cts[i]; m++) {
      if (tos[m] == to1) cti1 = m;
    ...
    //uuuu, unnecesarrily complicated? dont need to do for all cells!!!!
    float4 pi, pp;
    float4 pi2, pp2;
    int two_pts = 0;
    
    //do something so that it doesnt loop through all the cells :(((
    float dist = pt_to_pillar_dist(pillar_centers[n], cell_centers[i], pillar_dirs[n]) - pillar_rads[n];
    if (dist<2.0) {    
    closest_points_on_segments(cell_centers[i], pillar_centers[n],
                               cell_dirs[i], pillar_dirs[n],
                               cell_lens[i], pillar_lens[n],
                               &pi, &pp, &pi2, &pp2, &two_pts);
        
    //pi is either end1 or end2
    //if parallel, pi and pi2 = end1 and end2
    float4 v_ip = pp - pi; //vector between closest points
    float dist = length(v_ip) - (cell_rads[i]+pillar_rads[n]);
    float4 norm = normalize(v_ip); //normal on cell                        
    ...
    if ((two_pts) && (dist<0.f)){
      ...
      tos[cti1] = to1;
      dists[cti1] = dist;
      pts[cti1] = end1 + cell_rads[i]*norm;
      norms[cti1] = norm;
      reldists[cti1] = stiffness*pillar_coeffs[n]*dist;
      stiff[cti1] = stiffness*pillar_coeffs[n];      

      frs[cti2] = i;
      tos[cti2] = to2;
      dists[cti2] = dist;
      pts[cti2] = end2 + cell_rads[i]*norm;
      norms[cti2] = norm;
      reldists[cti2] = stiffness*pillar_coeffs[n]*dist;
      stiff[cti2] = stiffness*pillar_coeffs[n];
    }
    
    else if((!two_pts) && (dist<0.f)){
      float4 v1 = pi - end1;
      float4 v2 = pi - end2;
      
      if (min(length(v1), length(v2)) == length(v1)){
        if (cti1 < 0){
            cti1 = i*max_contacts +k;
            k++;
        }
      ...
      else if (min(length(v1), length(v2)) == length(v2)){
        if (cti2 < 0){
            cti2 = i*max_contacts +k;
            k++;
        }        
                           ...
                           __global float* stiff)
{
  ...
  // plane, pillar contacts have no to_ent, and have negative indices
  if (b < 0) {
    to_ents[i] = 0.f;
    return;
  }
