import PyQt5   #kk: changed PyQt4 to PyQt5
from PyQt5 import QtCore, QtGui, QtWidgets
from PyQt5.Qt import Qt
from PyQt5.QtCore import QObject, QTimer, pyqtSignal, pyqtSlot  #kk: removed QStringList, changed to list
...
class PyGLCMViewer(PyGLWidget):
    ...
    def __init__(self, parent = None):
        ...
        # Initial view setup
        self.set_radius(100)  #kk: changed display map size
    ...
    def getOpenCLPlatform(self):
        ...
        platlist = list([str(p.name) for p in platforms])
        ...
        #kk: changed QtGui to QtWidgets
        qsPlatformName, ok = QtWidgets.QInputDialog.getItem(self, \
    ...
    def getOpenCLDevice(self):
        ...
        devlist = list([str(d.name) for d in devices])
        ...
        #kk: changed QtGui to QtWidgets
        qsDeviceName, ok = QtWidgets.QInputDialog.getItem(self, \
    ...
    @pyqtSlot()
    def loadPickle(self):
        qs = QtWidgets.QFileDialog.getOpenFileName(self, 'Load pickle file', '', '*.pickle')  #kk:changed QtGui to QtWidgets
    ...
    @pyqtSlot()
    def load(self):
        qs = QtWidgets.QFileDialog.getOpenFileName(self, 'Load Python module', '', '*.py')  #kk: changed QtGui to QtWidgets
        if qs:
            modfile = str(qs)
    ...
    def paintGL(self):
        ...
        glBegin(GL_LINES)
        for i in range(5):
            glVertex(-50, (i-2)*25)   #kk: increased values by 2.5; change actual grid size
            glVertex(50, (i-2)*25)
            glVertex((i-2)*25, -50)
            glVertex((i-2)*25, 50)
        glEnd()
