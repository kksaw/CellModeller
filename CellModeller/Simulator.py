...
"""
    ...
    def __init__(self, \
        ...
        self.stepNum = 0
        self.lineage = {}
        self.ncells = {}  #kk: added
        self.pillarCtrs = {} #kk:added
    ...
    #kk: add many cells
    def addLawn(self, gridsize, cell_lens, frac, **kwargs):
        g = (gridsize)/(cell_lens*frac)
        G = int(ceil(g))
        for x in xrange(G+1):
            a = (x-g/2.00)*cell_lens*frac
            for y in xrange(G+1):
                b = (y-g/2.00)*cell_lens*frac
                
                n = 0
                k = 0
                while n < self.phys.n_pillars:
                    pctr = self.phys.pillar_centers[n]
                    prad = self.phys.pillar_rads[n]
                    if hypot(a-pctr[0], b-pctr[1])<prad:
                        k+=1
                    n+=1
                
                if k == 0:
                    self.addCell(pos=(a,b,0.5),
                                 dir=(random.random(), random.random(), random.random()),
                                 length=random.random()*cell_lens,
                                 rad=0.5)
                else:
                    pass
                    
    #kk: add many pillars                         
    def addPillars(self, dist, gridsize, rad):        
        d = dist/2.000 + rad
        xgrid = int(round((gridsize/(d*sqrt(3))),0)) + 2
        ygrid = int(round((gridsize/d),0)) + 2
        for x in xrange(xgrid):
            for y in xrange(ygrid):
                if (x%2==0 and y%2!=0) or (x%2!=0 and y%2==0):
                    self.phys.addPillar(((x-xgrid/2)*d*sqrt(3),(y-ygrid/2)*d), (0,0,1), rad, 50.0, 1.0)
    ...
    def writePickle(self, csv=False):
        ...
        data = {}
        data['ncells'] = self.phys.ncells  #kk:added
        data['pillarCtrs'] = numpy.array([self.phys.pillar_centers, self.phys.pillar_rads])
